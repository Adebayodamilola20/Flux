...
- [x] `Sources/App/DevNotchMain.swift` + `AppDelegate` — launches with no window
...
```sh
/usr/bin/log stream --predicate 'subsystem == "com.vinz.devnotch"' --level debug
```
...
DevNotch runs **no OAuth flow of its own**. It borrows a credential each tool
...
nothing to adjust: DevNotch runs no OAuth flow of its own, so it has no session
...
- [x] `WebSessionProvider.signOut()` clears its own cookies — the one true
      logout in the app, because that session is the only one DevNotch created.
...
- [ ] **Store notary credentials** — needs an app-specific password, so it has
      ...
      `xcrun notarytool store-credentials DevNotch --apple-id <id> --team-id 6WFPL8B9FB --password <app-specific-password>`
...
- [x] **Reset DevNotch…** in Settings: clears the defaults domain, caches,
...
DevNotch cannot switch the account. The credential belongs to Claude Code,
...
- [x] Reset is alone in its own section, and last. It used to sit a few pixels
      ...
      everything.
- [x] The "DevNotch never signs in" explanation moved into Integrations,
...
## Renaming to DevNotch

- [x] Every occurrence renamed, including the bundle identifier
      (`com.vinz.devnotch`), scheme, target, test target and log subsystem.
...
DevNotch 1.7.0 targets macOS 15, where `glassEffect` does not exist yet. A
...
## Decisions needed
- [ ] Final app name (`DevNotch` is a placeholder)
