...
# "Signing for DevNotch requires a development team", on every target.
...
run: build
	@APP=$$(xcodebuild -project $(PROJECT) -scheme $(SCHEME) -destination '$(DEST)' \
		...
		| awk -F' = ' '/ BUILT_PRODUCTS_DIR/ {print $$2; exit}')/DevNotch.app; \
	pkill -x DevNotch || true; \
	open "$$APP"
...
archive: gen
	...
	@# release leaves extra "DevNotch" entries in app search next to the
...
dmg: archive
	...
	@# three spare "DevNotch" entries end up in Spotlight; everything
...
SPARKLE_BIN = $(shell dirname $$(find $$HOME/Library/Developer/Xcode/DerivedData/DevNotch-*/SourcePackages/artifacts/sparkle -name generate_appcast 2>/dev/null | head -1))
...
DOWNLOAD_PREFIX := https://devnotch-ai.vercel.app/
...
build-ci: gen
	...
	@# "DevNotch" entries in Spotlight next to the installed app.
	@touch build/.metadata_never_index
	...
	@#
	@# which macOS reports to the user as "DevNotch cannot be opened because
