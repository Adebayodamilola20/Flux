...
final class OllamaRenderTests: XCTestCase {
    ...
    func testLiveLocalListingWhenExplicitlyEnabled() async throws {
        guard ProcessInfo.processInfo.environment["DEVNOTCH_OLLAMA_LIVE"] == "1" else {
        ...
        XCTAssertTrue(snapshot.hasReading)
        XCTAssertNil(snapshot.ringFraction)
        if let expectedName = ProcessInfo.processInfo.environment["DEVNOTCH_OLLAMA_EXPECTED_MODEL"],
           let expectedBrand = ProcessInfo.processInfo.environment["DEVNOTCH_OLLAMA_EXPECTED_BRAND"] {
