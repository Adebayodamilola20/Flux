...
/// The session DevNotch starts itself must never reach the notch.
///
...
/// carries the pid of the process DevNotch spawned. Left alone it draws a row
...
final class ClaudeOwnSessionFilterTests: XCTestCase {
    ...
    func testAnIgnoredPidIsLeftOut() throws {
        ...
        XCTAssertTrue(found.isEmpty, "the session DevNotch started is not the user's")
    }
