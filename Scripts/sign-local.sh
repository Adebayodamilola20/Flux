...
#
# Usage:
#   Scripts/sign-local.sh [path/to/DevNotch.app]
# Default target is the app in /Applications.

set -euo pipefail

APP="${1:-/Applications/DevNotch.app}"
CN="DevNotch Local Signing"
...
EOF
  ...
  openssl pkcs12 -export -legacy -inkey "$DIR/key.pem" -in "$DIR/cert.pem" \
    -out "$DIR/id.p12" -passout pass:devnotch -name "$CN" 2>/dev/null || \
  openssl pkcs12 -export -inkey "$DIR/key.pem" -in "$DIR/cert.pem" \
    -out "$DIR/id.p12" -passout pass:devnotch -name "$CN"
  ...
  security import "$DIR/id.p12" -k "$HOME/Library/Keychains/login.keychain-db" \
    -P devnotch -T /usr/bin/codesign -A
fi

# Quit a running instance so the bundle can be replaced in place.
osascript -e 'quit app "DevNotch"' 2>/dev/null || true
sleep 1
...
echo
echo "Signed with '$CN'. Relaunch DevNotch and grant the keychain prompt one"
