...
private struct VisualEffect: NSViewRepresentable {
    ...
    private func apply(to view: NSVisualEffectView, context: Context) {
        if context.environment.devnotchReduceTransparency {
...
struct SettingsView: View {
    ...
    @Environment(\.devnotchReduceTransparency) private var reduceTransparency
    ...
    var body: some View {
        ...
        .environment(\.devnotchAccentColor, preferences.accentColor.color)
    ...
    // One pane, because they are one question: what DevNotch looks like and
    // where it turns up. Split across several it read as unrelated settings,
    // and "Where DevNotch appears" was a header long enough to look like a
    // warning.
    ...
    private var notificationsPane: some View {
        Form {
            ...
            Section(L10n.t("When a session ends")) {
                ...
                Text(L10n.t("DevNotch already knows the moment an agent stops working or stops to ask you something. Clicking the notch while it is open brings that session's app to the front — the app, not the tab: only some terminals let anything outside them choose a tab, so the tooltip names the session instead."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    // Startup and updates together: both are about what DevNotch does
    ...
    private var generalPane: some View {
        Form {
            ...
            Section {
                Toggle(L10n.t("Open DevNotch at login"), isOn: $preferences.launchAtLogin)
                ...
                HStack(alignment: .firstTextBaseline, spacing: 10) {
                    ...
                    // hiding.
                    Text(L10n.t("Version \(updater.currentVersion). Updates install in the background and apply next time DevNotch starts."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    Spacer(minLength: 0)
            ...
            Section {
                HStack(spacing: 4) {
                    Text(L10n.t("Based on Codenotch by"))
    ...
    private var displayExplanation: String {
        ...
        case .display(let id):
            ...
            return L10n.t("That display is disconnected. DevNotch follows the active window until it returns.")
        }
    }
...
/// One provider: whether DevNotch reads it, whose account that is, and where
...
private struct AccountRow: View {
    ...
    private var accountDetail: some View {
        if let model = provider.localModel {
            ...
            Text(L10n.t("macOS is not letting DevNotch read \(provider.name)'s saved login. Choose Allow access… above, then Always Allow."))
                .foregroundStyle(.orange)
                .fixedSize(horizontal: false, vertical: true)
        } else {
            HStack(spacing: 8) {
