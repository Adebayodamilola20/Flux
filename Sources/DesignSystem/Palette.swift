...
private struct DevNotchReduceTransparencyKey: EnvironmentKey {
    static let defaultValue: Bool = false
}

extension EnvironmentValues {
    /// True when macOS Accessibility "Reduce Transparency" is enabled in system settings,
    /// or explicitly overridden via `.environment(\.devnotchReduceTransparency, ...)`.
    var devnotchReduceTransparency: Bool {
        get { self[DevNotchReduceTransparencyKey.self] || self.accessibilityReduceTransparency }
        set { self[DevNotchReduceTransparencyKey.self] = newValue }
    }
}
