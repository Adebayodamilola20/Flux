...
- [ ] `project.yml` for XcodeGen: app target `DevNotch` + unit test target
- [ ] `Makefile` with `gen` / `build` / `test` / `run` / `clean`
- [ ] `Sources/Info.plist` with `LSUIElement: true`, min system 26.0
- [ ] `Sources/App/DevNotchMain.swift` + `AppDelegate` that launches with no window
...
1. **Name.** `DevNotch` is a placeholder.
