...
this requirement and the listener's current status. Closing DevNotch closes
...
unlocked Mac's running DevNotch UI showed five distinct brand icons and their
...
This interpretation follows the current product: DevNotch monitors other tools.
...
Refresh animation only means DevNotch is fetching a reading. Model names take
...
in the [provider integration map](../../.agents/skills/devnotch-providers/references/provider-integration.md).
...
The tests use `TEST_RUNNER_DEVNOTCH_OLLAMA_LIVE=1` to opt into the live check and
...
`TEST_RUNNER_DEVNOTCH_OLLAMA_EXPECTED_MODEL` and
`TEST_RUNNER_DEVNOTCH_OLLAMA_EXPECTED_BRAND` (`qwen`, `gemma`, `llama`,
